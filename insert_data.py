...
def insert_user(username, mailAddress, token=None):
    try:
        cnx = mysql.connector.connect(**config)
        cursor = cnx.cursor()
        insert_query = "INSERT INTO User (username, mailAddress, token) VALUES (%s, %s, %s)"
        cursor.execute(insert_query, (username, mailAddress, token))
        cnx.commit()
        print("用户数据插入成功。")
...
# 示例数据插入
insert_user('user1234', 'example@example.com', token="V1an1337")
